...
proc checksum*(mmp: MmpFile): string =
  ...
  # block checkSumMmpFile:
  #   var h: Hash = 0
  #   mmp.mapFiles.description = "foo"
  #   h = h !& mmp.mmpVersion
  #   h = h !& hash(mmp.mapFiles.description)
  #   h = h !& hash(mmp.mapFiles.gciFile)
  #   h = h !& hash(mmp.mapFiles.gmpFile)
  #   h = h !& hash(mmp.mapFiles.gxtFile)
  #   h = h !& hash(mmp.mapFiles.mdFile)
  #   h = h !& hash(mmp.mapFiles.misPath)
  #   h = h !& hash(mmp.mapFiles.scrFile)
  #   h = h !& hash(mmp.mapFiles.styFile)
  #   h = h !& hash(mmp.mapFiles.playerCount)
  #   h = !$h
  #   echo $h
...
# import zip/zipfiles
#
# proc packageMap*(mmp: MmpFile) =
#   mmp.validate()
#   let files = toSeq(mmp.files())
#
#   var z: ZipArchive
...
proc package*(mmp: MmpFile, writeMmpStanza = true): string =
  ...
  let
    ...
    # chksum = fullChksum[0..12]
    # chksum = fullChksum[0..15]
    mapFileName = fmt"rls-{chksum}"

  assert (mapFileName & ".mmp").len == 22
  # assert mapFileName.len == 21
  ...
  # var
  #   mmpFile: tuple[kind: string, path: string, fileName: string]
  ...
  for file in mmp.files(skipMmpItself=true):
    ...
    if file.fileName.endsWith(".scr"):  # @XXX @TODO replace with 'MISFile' at a later point
      ...
      # files.add((path: misPath, fileName: os.extractFilename(misPath)))
      # files.add((path: misPath, fileName: mapFileName & ".mis"))
  ...
  block hotpatch_mmp:
    ...
    # strm.writeLine(fmt"FullChecksum = {fullChksum}")
